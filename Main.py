...
def assumed_fill(game: Game) -> tuple[bool]:
    ...
    while fill_algorithm.count_items_remaining():
        ...
        if fill_algorithm.count_items_remaining() == 0:
            ...
            completable = (game.all_locations["E2 Morph Ball"]["item"] == Items.Morph or
                           game.all_locations["E2 Morph Ball"]["item"] == Items.Screw or
                           game.all_locations["E2 Morph Ball"]["item"] == Items.Super or
                           game.all_locations["C2 Missile"]["item"] == Items.Missile or
                           game.all_locations["C2 Missile"]["item"] == Items.Super or
                           game.all_locations["C2 Missile"]["item"] == Items.Morph or
                           game.all_locations["C2 Missile"]["item"] == Items.Screw)
...
def write_rom(game: Game, romWriter: Optional[RomWriter] = None) -> str:
    ...
    rom_name = f"Cube{game.seed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/Digital Cube.sfc"
    ...
    # 10 missiles open
    romWriter.writeBytes(0x260dd, b"\x0a")
    # 10 super missiles open
    romWriter.writeBytes(0x26102, b"\x0a")
    # 10 power bombs open
    romWriter.writeBytes(0x26127, b"\x0a")
    # 10 missiles chozo
    romWriter.writeBytes(0x264a6, b"\x0a")
    # 10 super missiles chozo
    romWriter.writeBytes(0x264d8, b"\x0a")
    # 10 power bombs chozo
    romWriter.writeBytes(0x2650a, b"\x0a")
    # 10 missiles hidden
    romWriter.writeBytes(0x26977, b"\x0a")
    # 10 super missiles hidden
    romWriter.writeBytes(0x269af, b"\x0a")
    # 10 power bombs hidden
    romWriter.writeBytes(0x269e7, b"\x0a")
    # Alpha Missile red door removed for start game
    romWriter.writeBytes(0x7c636, b"\x46")
